...
//		Integer role = 0;
		if (role == null || role.toString().isEmpty()) {
			...
			if(hasUpdateFlag==0){
                ...
                String HQL_QUERY = "select r.rolenum from com.egov.secrecysystem.model.CpRole r where r.rolenum is not null group by r.rolenum";
				List<Integer> roleNum =session.createQuery(HQL_QUERY).list();
				for(Integer element:roleNum){
					HQL_QUERY="select t.onclickscript from com.egov.secrecysystem.model.CpMainmenu t where t.cpMainmenu is not null and t.onclickscript like '%.jsp' and  t.cpRole.rolenum="+element;
