...
function renderBold(value) {
	return "<span style='font-weight:bold;font-size:20px'>" + value + "</span>";
}

// 渲染答题情况背景
function renderBgColor(value) {
	if(value == "正确") {
		return "<span style='color:green;font-weight:bold;font-size:20px'>" + value + "</span>";
	} else if(value == "错误") {
		return "<span style='color:#FF0000;font-weight:bold;font-size:20px'>" + value + "</span>";
	} else if(value =="待定" ) {
		return "<span style='color:#FF6633;font-weight:bold;font-size:20px'>" + value + "</span>";
	} else {
...
function showRank(value, metaData, record, rowIndex, colIndex, store) {
	if(rowIndex == 0) {
		return "<span style='color:green;font-weight:bold;font-size:20px'>" + "第一名" + "</span>";
	} else if(rowIndex ==1) {
		return "<span style='color:green;font-weight:bold;font-size:20px'>" + "第二名" + "</span>";
	} else if(rowIndex ==2) {
		return "<span style='color:green;font-weight:bold;font-size:20px'>" + "第三名" + "</span>";
	} else {
		return;
	}
}
...
function WindowInit() {
	...
	cmItems.push({
		id : 'name',
		header : "<span style='font-weight:bold;font-size:20px'>" + "名称" + "</span>",
		dataIndex : 'name',
		width : 120,
		sortable : true,
		renderer : renderBold
	...
	cmItems.push({
		id : 'rank',
		header : "<span style='font-weight:bold;font-size:20px'>" + "排名" + "</span>",
		dataIndex : 'rank',
		width : 80,
		renderer : showRank
	});
	...
	cmItems.push({
		id : 'score',
		header : "<span style='font-weight:bold;font-size:20px'>" + "总分" + "</span>",
		dataIndex : 'score',
		width : 60,
		sortable : true,
	...
	for (var i = 1; i <= questionCount; i++) {
		var cmConfig = {
			id : '' + i,
			header : "<span style='font-weight:bold;font-size:20px'>" + i + "</span>",
			dataIndex : '' + i,
			width : 53,
			sortable : true,
